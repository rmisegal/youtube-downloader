...
- [ ] RED: failing test for `RateLimiter` allows N requests/min then blocks (Plan Phase 4.1, Rule #7)
- [ ] RED: failing test for `RateLimiter` reads limits from config (Plan Phase 4.1, R27, Rule #7)
- [ ] RED: failing test for `RateLimiter` burst window behavior (Plan Phase 4.1, Rule #7)
- [ ] GREEN: implement `shared/rate_limit.py` `RateLimiter` (sliding-window/token-bucket) (Plan Phase 4.1, R27, Rule #8,#10)
- [ ] REFACTOR: extract window math helper; tests green (Plan Phase 4.1, Rule #2,#8)
- [ ] RED: failing test for `DownloadQueue` FIFO ordering (Plan Phase 4.2, Rule #7)
- [ ] RED: failing test for `DownloadQueue` max_depth overflow (queued, not crash) (Plan Phase 4.2, R28, Rule #7)
- [ ] RED: failing test for `DownloadQueue` overflow_strategy `reject_oldest` (Plan Phase 4.2, R28, Rule #7)
- [ ] RED: failing test for `DownloadQueue` drain (Plan Phase 4.2, Rule #7)
- [ ] GREEN: implement `shared/queue.py` `DownloadQueue` FIFO + max_depth + drain + strategy (Plan Phase 4.2, R28, Rule #5,#8)
- [ ] REFACTOR: simplify queue state; tests green (Plan Phase 4.2, Rule #2,#8)
- [ ] RED: failing test for `ApiGatekeeper.execute` rate-check before call (Plan Phase 4.3, Rule #7)
- [ ] RED: failing test for `ApiGatekeeper` retry/backoff on transient error (Plan Phase 4.3, R26, Rule #7)
- [ ] RED: failing test for `ApiGatekeeper` max_retries exhaustion raises (Plan Phase 4.3, Rule #7)
- [ ] RED: failing test for `ApiGatekeeper` logs each call (Plan Phase 4.3, Rule #7)
- [ ] RED: failing test for `ApiGatekeeper` overflow enqueues to `DownloadQueue` (Plan Phase 4.3, R28, Rule #7)
- [ ] GREEN: implement `shared/gatekeeper.py` `ApiGatekeeper.execute(callable, ...)` (Plan Phase 4.3, R26, Rule #3,#8)
- [ ] GREEN: wire gatekeeper to `RateLimiter`, retry, logging, queue (Plan Phase 4.3, Rule #3,#5)
- [ ] REFACTOR: extract retry/backoff helper; tests green (Plan Phase 4.3, Rule #2,#8)
- [ ] RED: failing test for `FfmpegLocator.exe_dir()` returns dir of ffmpeg exe (Plan Phase 4.4, Rule #7)
- [ ] GREEN: implement `infra/ffmpeg.py` `FfmpegLocator` wrapping `imageio_ffmpeg.get_ffmpeg_exe()` (Plan Phase 4.4, R20, Rule #8)
- [ ] REFACTOR: cache located ffmpeg path; tests green (Plan Phase 4.4, Rule #2)
- [ ] RED: failing test for `ytdlp_client` routes `extract_info` via gatekeeper (Plan Phase 4.5, R26, Rule #7)
- [ ] RED: failing test for `ytdlp_client` routes `download` via gatekeeper (Plan Phase 4.5, R26, Rule #7)
- [ ] GREEN: implement `infra/ytdlp_client.py` thin `yt_dlp.YoutubeDL` wrapper (Plan Phase 4.5, R19, Rule #8)
- [ ] GREEN: ensure all net calls go through `ApiGatekeeper` (Plan Phase 4.5, R26, Rule #3)
- [ ] REFACTOR: dedupe option-dict construction; tests green (Plan Phase 4.5, Rule #2)
- [ ] RED: failing test for `BaseDownloader` builds shared ydl-opts/outtmpl (Plan Phase 4.6, Rule #7)
- [ ] RED: failing test for `BaseDownloader` wires `ffmpeg_location` (Plan Phase 4.6, R20, Rule #7)
- [ ] GREEN: implement `services/base.py` `BaseDownloader` shared opts builder (Plan Phase 4.6, R25, Rule #2,#8)
- [ ] REFACTOR: ensure subclasses override only specifics; tests green (Plan Phase 4.6, Rule #2)
- [ ] RED: failing test that opts builder injects `proxy` from `YTDL_PROXY` env, omitted when unset (PRD §6.4, Plan Phase 4.7, R44, Rule #7)
- [ ] RED: failing test that opts builder injects `cookiefile` from `YTDL_COOKIES_FILE` env, omitted when unset (PRD §6.4, Plan Phase 4.7, R44, Rule #7)
- [ ] GREEN: implement env-driven `proxy`/`cookiefile` injection in opts builder (PRD §6.4, Plan Phase 4.7, R44, Rule #8,#11)
- [ ] Phase 4 Gate — proxy/cookies consumed only from env when set, omitted when unset (Plan Gate 4 functional, R44, PRD §6.4)
- [ ] Phase 4 Gate — rate limit enforced; overflow queued not crashed (Plan Gate 4 functional)
- [ ] Phase 4 Gate — retries on transient error verified (Plan Gate 4 functional)
- [ ] Phase 4 Gate — grep: no direct `YoutubeDL(`/`requests.` outside `infra/` (Plan Gate 4, Rule #1,#3)
- [ ] Phase 4 Gate — confirm `BaseDownloader` removes duplication (Plan Gate 4, Rule #2)
- [ ] Phase 4 Gate — run `uv run pytest tests/unit/shared tests/unit/infra` (Plan Gate 4 automated)
- [ ] Phase 4 Gate — run ruff + file-size + coverage checks (Plan Gate 4, Rule #8,#9,#10)
- [ ] Phase 4 Commit — `feat(infra): gatekeeper, rate limiter, queue, ffmpeg, base downloader` (Rule #6)
...
- [ ] RED: failing test for `MetadataService` resolves info dict / title (Plan Phase 5.1, Rule #7)
- [ ] GREEN: implement `services/metadata.py` info/title extraction (Plan Phase 5.1, R12, Rule #8)
- [ ] REFACTOR: trim metadata mapping; tests green (Plan Phase 5.1, Rule #2)
- [ ] RED: failing test for `VideoDownloader` builds `bv*[height<=RES]+ba/b` (Plan Phase 5.2, R2, Rule #7)
- [ ] RED: failing test for `VideoDownloader` sets `merge_output_format="mp4"` (Plan Phase 5.2, R2, Rule #7)
- [ ] RED: failing test for `VideoDownloader` no-resolution → best (`bv*+ba/b`) (Plan Phase 5.2, R8, Rule #7)
- [ ] GREEN: implement `services/video.py` `VideoDownloader` (Plan Phase 5.2, R2, Rule #8)
- [ ] REFACTOR: clean format-string builder; tests green (Plan Phase 5.2, Rule #2)
- [ ] RED: failing test for `AudioDownloader` adds `FFmpegExtractAudio` postprocessor (Plan Phase 5.3, R3, Rule #7)
- [ ] RED: failing test for `AudioDownloader` codec/quality read from config (Plan Phase 5.3, R3, Rule #11)
- [ ] GREEN: implement `services/audio.py` `AudioDownloader` (Plan Phase 5.3, R3, Rule #8)
- [ ] REFACTOR: dedupe postprocessor list builder; tests green (Plan Phase 5.3, Rule #2)
- [ ] RED: failing test for `SubtitleDownloader` sets writesubtitles+writeautomaticsub (Plan Phase 5.4, R4, Rule #7)
- [ ] RED: failing test for `SubtitleDownloader` `subtitleslangs=[lang]` (Plan Phase 5.4, R9, Rule #7)
- [ ] RED: failing test for `SubtitleDownloader` `FFmpegSubtitlesConvertor` → srt (Plan Phase 5.4, R11, Rule #7)
- [ ] RED: failing test for subtitle output named `<name>.<lang>.srt` (Plan Phase 5.4, R11, Rule #7)
- [ ] RED: failing test for auto-only-captions fallback path (Plan §3 R2 risk, Rule #7)
- [ ] GREEN: implement `services/subtitles.py` `SubtitleDownloader` (Plan Phase 5.4, R4, Rule #8)
- [ ] REFACTOR: share subtitle opts via base; tests green (Plan Phase 5.4, Rule #2)
- [ ] Write docstrings for all four service classes (Plan §4, Rule #1)
- [ ] Phase 5 Gate — each service builds correct ydl-opts (mocked) (Plan Gate 5 functional)
- [ ] Phase 5 Gate — subtitle convertor guarantees `.srt`; manual+auto covered (Plan Gate 5 functional)
- [ ] Phase 5 Gate — confirm subclasses override only specifics, no dup opts (Plan Gate 5, Rule #2)
- [ ] Phase 5 Gate — run `uv run pytest tests/unit/services` (Plan Gate 5 automated)
- [ ] Phase 5 Gate — run ruff + file-size + coverage checks (Plan Gate 5, Rule #8,#9,#10)
- [ ] Phase 5 Commit — `feat(services): video, audio, subtitle, metadata downloaders` (Rule #6)
...
- [ ] RED: failing test for positional `url` parsing (Plan Phase 7.1, R1, Rule #7)
- [ ] RED: failing test for `--video/--audio/--subs` flag parsing (Plan Phase 7.1, R2,R3,R4, Rule #7)
- [ ] RED: failing test for `-o/--output-dir` parsing (Plan Phase 7.1, R6, Rule #7)
- [ ] RED: failing test for `-n/--name` parsing + default `%(title)s` (Plan Phase 7.1, R7, Rule #7)
- [ ] RED: failing test for `--resolution` parsing (Plan Phase 7.1, R8, Rule #7)
- [ ] RED: failing test for `--sub-lang` parsing + default `en` (Plan Phase 7.1, R9, Rule #7)
- [ ] RED: failing test for `--version` prints code+config version (Plan Phase 7.1, R10, Rule #7)
- [ ] RED: failing test that no output flag → defaults to video (Plan Phase 7.2, R5, Rule #7)
- [ ] RED: failing test that combinable toggles pass through to SDK (Plan Phase 7.2, R5, Rule #7)
- [ ] RED: failing test for UTF-8 stdout reconfiguration (Plan Phase 7.3, R13, Rule #7)
- [ ] RED: failing test for structured progress logging per phase (Plan Phase 7.3, R14, Rule #7)
- [ ] RED: failing test for idempotent output-dir creation (Plan Phase 7.4, R16, Rule #7)
- [ ] RED: failing test for deterministic exit codes (success/bad-url/version-mismatch) (Plan Phase 7.4, R15, Rule #7)
- [ ] RED: failing test for non-zero exit on network failure after retries (PRD §3.3, Plan Phase 7.4, R15, Rule #7)
- [ ] RED: failing test for non-zero exit on unsupported request (PRD §3.3, Plan Phase 7.4, R15, Rule #7)
- [ ] RED: failing test that CLI is non-interactive (no prompts) (Plan Phase 7.4, R17, Rule #7)
- [ ] GREEN: implement `cli/main.py` argparse with all flags (Plan Phase 7.1, R1,R6-R10, Rule #8)
- [ ] GREEN: implement default-to-video + combinable toggles (Plan Phase 7.2, R5)
- [ ] GREEN: implement UTF-8 stdout reconfig (Plan Phase 7.3, R13)
- [ ] GREEN: implement structured progress logging (Plan Phase 7.3, R14)
- [ ] GREEN: implement idempotent dir creation (Plan Phase 7.4, R16)
- [ ] GREEN: implement deterministic exit codes for all four conditions — invalid URL, network-failure-after-retries, unsupported request, config-version mismatch (PRD §3.3, Plan Phase 7.4, R15)
- [ ] GREEN: ensure CLI delegates 100% to SDK (no business logic) (Plan Phase 7, R24, Rule #1)
- [ ] GREEN: implement `src/main.py` shim for `python -m ytdl` (Plan Phase 7.5, A2)
- [ ] REFACTOR: extract arg→SDK mapping helper; tests green (Plan Phase 7, Rule #2)
- [ ] Write `--help` text and docstrings for CLI (Plan §3.1, Rule #1)
- [ ] Phase 7 Gate — all flags parse; `--version` prints versions (Plan Gate 7 functional)
- [ ] Phase 7 Gate — missing dir auto-created; non-zero exit on bad URL/version (Plan Gate 7 functional)
- [ ] Phase 7 Gate — confirm zero business logic in CLI (delegates to SDK) (Plan Gate 7, Rule #1)
- [ ] Phase 7 Gate — run `uv run pytest tests/unit/cli` (Plan Gate 7 automated)
- [ ] Phase 7 Gate — run ruff + file-size + coverage checks (Plan Gate 7, Rule #8,#9,#10)
- [ ] Phase 7 Commit — `feat(cli): argparse interface delegating to SDK` (Rule #1,#6)
...
- [ ] Implement `tests/conftest.py` `mock_ytdl` fixture (Plan Phase 8.1, R39, Rule #7)
- [ ] Implement `conftest.py` mock `imageio_ffmpeg`/`subprocess` fixtures (Plan Phase 8.1, R39, Rule #7)
- [ ] Implement `conftest.py` `sample_config` fixture (Plan Phase 8.1, R39)
- [ ] Implement `conftest.py` `tmp_output_dir` fixture (Plan Phase 8.1, R39)
- [ ] Add error-path test: invalid URL (Plan Phase 8.2, R39, Rule #7)
- [ ] Add error-path test: empty/`None` args (Plan Phase 8.2, R39, Rule #7)
- [ ] Add error-path test: rate-limit overflow → queued (Plan Phase 8.2, R28, Rule #7)
- [ ] Add error-path test: config-version mismatch (Plan Phase 8.2, R34, Rule #7)
- [ ] Confirm no network and no real ffmpeg invoked in unit tests (Plan Gate 8 functional, R39)
- [ ] Run coverage; raise gaps to ≥85% (Plan Phase 8.3, R38, Rule #9)
- [ ] Confirm test files also ≤150 LOC (Plan Phase 8.3, Rule #8)
- [ ] Phase 8 Gate — full suite green (Plan Gate 8 functional)
- [ ] Phase 8 Gate — coverage ≥85% enforced by `fail_under` (Plan Gate 8, Rule #9)
- [ ] Phase 8 Gate — confirm happy+error per public method (Plan Gate 8, Rule #7)
- [ ] Phase 8 Gate — run `uv run pytest tests/ --cov=src --cov-report=term-missing` (Plan Gate 8 automated)
- [ ] Phase 8 Gate — run ruff + file-size checks (Plan Gate 8, Rule #8,#10)
- [ ] Phase 8 Commit — `test: fixtures, error paths, coverage >=85%` (Rule #6,#9)
...
- [ ] Run `uv sync` (Plan Phase 11.1, R42)
- [ ] Run `uv run ruff check src/ tests/` zero violations (Plan Phase 11.1, Rule #10)
- [ ] Run `uv run pytest tests/ --cov=src --cov-report=term-missing` (Plan Phase 11.1, Rule #9, R42)
- [ ] Run file-size script, 0 files > 150 LOC (Plan Phase 11.1, Rule #8)
- [ ] Run secret-scan script clean (Plan Phase 11.1, Rule #13)
- [ ] Manual smoke test: `uv run python -m ytdl "<URL>" --video --audio --subs -o .\downloads -n sample --resolution 720` (Plan Phase 11.2, R18, S1)
- [ ] Verify `sample.mp4` produced and plays (Plan Phase 11.2, R2, S1)
- [ ] Verify `sample.mp3` produced and plays (Plan Phase 11.2, R3, S1)
- [ ] Verify `sample.en.srt` produced and opens (Plan Phase 11.2, R4, S1)
- [ ] Confirm no system FFmpeg / no GPU needed (Plan Phase 11.2, R18, S2)
- [ ] Confirm non-goals respected: no GUI/web (Plan Phase 11.3, R40)
- [ ] Confirm non-goals respected: no playlist CLI (Plan Phase 11.3, R40)
- [ ] Confirm non-goals respected: no DRM bypass (Plan Phase 11.3, R40)
- [ ] Complete Sign-Off: 100% PRD coverage (Plan §11 sign-off, S3)
- [ ] Complete Sign-Off: 14/14 rules PASS (Plan §10, S3)
- [ ] Complete Sign-Off: no open questions (Plan §8)
- [ ] Phase 11 Gate — S1–S3 met; smoke artifacts valid (Plan Gate 11 functional)
- [ ] Phase 11 Commit — `chore(release): v1.00 verification + sign-off` (Rule #6)
